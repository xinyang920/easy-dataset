...
function createWindow() {
  ...
  if (isDev) {
    mainWindow.loadURL(`http://localhost:${port}/easydataset`);
...
async function startNextServer() {
  ...
  if (isPortBusy) {
    ...
    return `http://localhost:${port}/easydataset`;
  }
  ...
  try {
    ...
    const server = http.createServer((req, res) => {
      // 修改请求路径，添加/easydataset前缀
      const parsedUrl = url.parse(req.url, true);
      const { pathname, query } = parsedUrl;
      
      if (pathname.startsWith('/easydataset')) {
        handle(req, res, {
          ...parsedUrl,
          pathname: pathname.replace('/easydataset', '') || '/',
          query
        });
      } else {
        // 重定向到/easydataset
        res.writeHead(302, {
          Location: '/easydataset'
        });
        res.end();
      }
    });
    ...
    return new Promise((resolve) => {
      server.listen(port, (err) => {
        ...
        resolve(`http://localhost:${port}/easydataset`);
      });
    });
  } catch (error) {
